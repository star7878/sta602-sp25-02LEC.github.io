...
(b). Given the data on the previous slide, standardize $y$ and $x$. Next, using Monte Carlo simulation, report $E[\theta | y_1,\ldots y_n]$ together with 95% posterior confidence intervals.
...
y = rnorm(N, true.theta * x, true.sigma) + 10
...
kn = k_0 + sumX2
nun = nu_0 + n
mun = ((k_0 * mu_0) + sumXY) / (kn)
betaN = (sumY2 - (k_0 * (mu_0^2))) + ((sumXY + (k_0 * mu_0))^2 / (sumX2 + k_0))
betaN = betaN / nun
theta.postsample = rnorm(10000, mun, sqrt(1 / kn))
