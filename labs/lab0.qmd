...
5. An example of plotting pdfs with `ggplot` (loaded via the `tidyverse` package.)
...
Alternatively, if we wish to plot the density of a pmf, here's another example

```{r}
df2 = data.frame(x = c(0:6))
df2 %>%
  mutate(rate1 = dpois(x, 2)) %>%
  ggplot(aes(x = x, y = rate1)) + 
