...
c. Report (using Monte Carlo sampling other otherwise) the posterior mean of the [relative risk](https://en.wikipedia.org/wiki/Relative_risk), i.e. the posterior mean of the probability of stroke in the treatment group versus in the control, (think $E~[\frac{\theta_t}{\theta_c}~|~\text{data}]$). Additionally, include a 95% posterior confidence interval for the relative risk.
