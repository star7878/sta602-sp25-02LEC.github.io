...
- Find the unit information prior $p(\theta)$. Hint: find a density $p(\theta)$ such that $\log p(\thtea) = l(\theta | y_1,\ldots,y_n) + c$ where $c$ is a constant that doesn't depend on $\theta$. 
:::
